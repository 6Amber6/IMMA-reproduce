#!/bin/bash
#SBATCH --job-name=imma_pgd_restart
...
# Backup any previous adv dir with timestamp
if [ -d data/vangogh_adv ]; then
    BACKUP_NAME="data/vangogh_adv_backup_$(date +%Y%m%d_%H%M%S)"
    mv data/vangogh_adv "$BACKUP_NAME"
    echo "Backed up old adv dir to $BACKUP_NAME"
fi
...
echo ""
echo "=== Attack stats ==="
cat data/vangogh_adv/attack_stats.csv
